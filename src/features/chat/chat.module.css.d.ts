declare const styles: {
  readonly "button": string;
  readonly "chat-container": string;
  readonly "chat-footer": string;
  readonly "chat-form": string;
  readonly "chat-header": string;
  readonly "chat-page": string;
  readonly "chat-submit-button": string;
  readonly "chat-text-field": string;
  readonly "error-message": string;
  readonly "logout-button": string;
  readonly "message-author": string;
  readonly "message-date": string;
  readonly "message-item": string;
  readonly "message-list": string;
  readonly "message-list-wrapper": string;
  readonly "register": string;
  readonly "register-submit-button": string;
  readonly "register-textfield": string;
  readonly "sender": string;
};
export = styles;

